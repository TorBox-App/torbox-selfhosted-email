...
describe("findHostedZone", () => {
  ...
  it("should query Route53 with correct parameters", async () => {
    ...
    expect(route53Mock.commandCalls(ListHostedZonesByNameCommand)).toHaveLength(
      1
    );
    expect(
      route53Mock.commandCalls(ListHostedZonesByNameCommand)[0].args[0].input
    ).toEqual({
      DNSName: "test.com",
      MaxItems: 1,
    });
...
describe("createDNSRecords", () => {
  ...
  it("should create correct DKIM CNAME records", async () => {
    ...
    expect(dkimRecords?.[1].ResourceRecordSet?.Name).toBe(
      "def._domainkey.example.com"
    );
