 ...
 *
 * This script copies pre-bundled Lambda functions from @wraps/core into the
 * dist/lambda directory so they're available when the package is published.
 *
 * The @wraps/core package is bundled into this package (noExternal), but import.meta.url
...
main().catch((error) => {
  console.error("Failed to copy Lambda functions:", error);
  process.exit(1);
});
