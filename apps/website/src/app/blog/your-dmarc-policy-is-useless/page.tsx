...
// - $125K average BEC loss: FBI IC3 2024 ($2.77B / ~22,000 reported incidents)
// - 60% of BEC involves domain impersonation: Security research cited in Toyota case
const AttackCostCalculator = () => {
  const [employees, setEmployees] = useState(100);
  const [hasEnforcement, setHasEnforcement] = useState(false);

  const becSuccessRate = hasEnforcement ? 0.001 : 0.02;
  const avgBecLoss = 125_000; // FBI IC3: $2.77B / ~22K incidents
  const dataBreachCost = 4_880_000; // IBM 2024
  const breachProbability = hasEnforcement ? 0.005 : 0.08;

  const annualRisk =
    employees * becSuccessRate * avgBecLoss +
    breachProbability * dataBreachCost;
  const dmarcCost = hasEnforcement ? 500 * 12 : 0;
  const netSavings = hasEnforcement
    ? employees * 0.02 * avgBecLoss +
      0.08 * dataBreachCost -
      dmarcCost -
      annualRisk
    : 0;
  ...
  return (
    <Card className="p-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-4">
          ...
          <div className="mt-4 flex items-center gap-3">
            <button
              className={`relative h-7 w-14 rounded-full transition-colors ${hasEnforcement ? "bg-green-600" : "bg-red-600"}`}
              onClick={() => setHasEnforcement(!hasEnforcement)}
            >
              <div
                className={`absolute top-1 h-5 w-5 rounded-full bg-white transition-transform ${hasEnforcement ? "translate-x-8" : "translate-x-1"}`}
              />
            </button>
        ...
        <div className="space-y-3">
          <div
            className={`rounded-lg p-4 ${hasEnforcement ? "border border-green-500/50 bg-green-500/10" : "border border-red-500/50 bg-red-500/10"}`}
          >
            <div className="text-muted-foreground text-sm">
              Estimated Annual Risk
            </div>
            <div
              className={`font-bold text-3xl ${hasEnforcement ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
            >
              ${Math.round(annualRisk).toLocaleString()}
            </div>
          </div>
          ...
          {hasEnforcement && (
            <div className="rounded-lg border border-green-500/50 bg-green-500/10 p-3">
              <div className="text-muted-foreground text-xs">
                Annual Savings with Enforcement
              </div>
              <div className="font-bold text-green-600 text-xl dark:text-green-400">
      ...
      <div className="mt-4 border-t pt-4 text-muted-foreground text-xs">
        <span className="font-medium">Sources:</span> Average BEC loss from{" "}
        <a
          className="text-primary hover:underline"
          href="https://www.ic3.gov/Media/PDF/AnnualReport/2024_IC3Report.pdf"
          rel="noopener noreferrer"
          target="_blank"
        >
          FBI IC3 2024 Report
        </a>{" "}
        ($2.77B total). Data breach cost from{" "}
        <a
          className="text-primary hover:underline"
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-4xl space-y-24 px-4 pt-24 pb-24">
        ...
        <section>
          <div className="prose prose-neutral dark:prose-invert mb-6 max-w-none">
            <h2 className="mb-4 font-bold text-3xl">
              What would a BEC attack cost you?
            </h2>
            <p className="text-foreground/80 text-xl leading-relaxed">
              BEC attacks succeed because employees trust emails that appear to
              come from colleagues. The average loss scales with company
              size—but so does the target value. Use the calculator below to
              estimate your exposure based on typical attack patterns.
            </p>
          </div>
