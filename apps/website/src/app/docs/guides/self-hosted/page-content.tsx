...
# Set when --web-domain flag is provided:
SELFHOST_WEB_DOMAIN=dashboard.yourdomain.com

# Populated after SST deploy completes:
# Without --web-domain, NEXT_PUBLIC_APP_URL is a *.cloudfront.net URL:
NEXT_PUBLIC_APP_URL=https://d1abc123xyz.cloudfront.net
WRAPS_API_URL=https://abc123.lambda-url.us-east-1.on.aws
BETTER_AUTH_URL=https://d1abc123xyz.cloudfront.net
    ...
    <DocsLayout>
      ...
      <section className="mb-12">
        ...
        <p className="mb-4 text-muted-foreground">
          ...
          dashboard, SQS workers, DynamoDB, and EventBridge Scheduler — into
          your AWS account using{" "}
          <a
          ...
          . You fork the Wraps repository, configure a handful of secrets, and
          run a single command. Everything runs in your account; no Wraps SaaS
          servers are in the critical path.
        </p>
        <Card>
        ...
        <div className="mt-4 rounded-lg border-primary border-l-4 bg-primary/10 p-4">
          ...
          <p className="mt-2 text-muted-foreground text-sm">
            ...
            from a fork (this guide) deploys the{" "}
            <span className="font-medium text-foreground">full platform</span>{" "}
            via SST: API Lambda, the dashboard on CloudFront, workers, DynamoDB,
...
pnpm install`,
      ...
      <section className="mb-12">
        ...
        <div className="rounded-lg border-primary border-l-4 bg-primary/10 p-4">
          ...
          <p className="mt-2 text-muted-foreground text-sm">
            Pass{" "}
            <code className="rounded bg-muted px-1.5 py-0.5">--region</code> to
            deploy somewhere other than{" "}
            ...
            reads it back — so the region is set once, not re-passed. No file
            editing required.
          </p>
        </div>
      ...
      <section className="mb-12">
        ...
        <ul className="space-y-2 text-muted-foreground text-sm pl-4 list-disc">
          <li>
            <code className="rounded bg-muted px-1.5 py-0.5">
              wraps selfhost update-role
            </code>{" "}
            — refreshes the permissions on{" "}
            <code className="rounded bg-muted px-1.5 py-0.5">
              wraps-selfhost-console-access-role
            </code>{" "}
            after an upgrade adds new ones. It only ever writes the self-hosted
            role;{" "}
            <code className="rounded bg-muted px-1.5 py-0.5">
              wraps platform update-role
            </code>{" "}
            is the equivalent for the platform role and the two leave each other
            alone.
          </li>
          <li>
            <code className="rounded bg-muted px-1.5 py-0.5">
      ...
      <section className="mb-12">
        ...
        <p className="mb-4 text-muted-foreground">
          A healthy deployment reports the API URL, app URL, AWS region,
          deployment timestamp, and license key prefix. Open your dashboard URL
        ...
        <Card className="mb-6">
          ...
          <CardContent className="pt-0">
            <p className="mb-3 text-muted-foreground text-sm">
              ...
              <a
                className="text-primary underline underline-offset-2"
                href="https://docs.github.com/en/actions/security-for-github-actions/security-hardening-your-deployments/configuring-openid-connect-in-amazon-web-services"
                rel="noopener noreferrer"
                target="_blank"
              >
              ...
              for full context. In the AWS Console, go to{" "}
              <span className="font-medium text-foreground">
            ...
            <p className="text-muted-foreground text-sm">
              Attach an IAM permissions policy to this role with the permissions
              SST needs: Lambda, S3, CloudFormation, IAM, SSM Parameter Store,
              CloudWatch Logs, STS, CloudFront, SQS, DynamoDB, and EventBridge
              Scheduler. Copy the role ARN — you will need it in the next step.
            </p>
            <p className="mt-2 text-muted-foreground text-sm">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="text-muted-foreground">
                  <tr className="border-b">
                    ...
                    <td className="py-2">
                      AWS region for deployments. The workflow writes it into
                      the reconstructed{" "}
                      <code className="rounded bg-muted px-1.5 py-0.5">
                        .env.selfhost
                      </code>{" "}
                      ...
                      — set it to the region you deployed into locally, or CI
                      will target a different one
                    </td>
                  </tr>
                  <tr className="border-b">
                    <td className="py-2 pr-4">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y text-muted-foreground">
              ...
              <tr>
                ...
                <td className="p-3 pr-4 align-top text-foreground">Yes</td>
                <td className="p-3 align-top">
                  AWS region the SST stack deploys into — written on first
                  deploy from{" "}
                  <code className="rounded bg-muted px-1.5 py-0.5 text-xs">
                    --region
                  </code>{" "}
                  ...
                  ) and read back by every later upgrade. Changing it after
                  deploy strands the existing stack in the old region
                </td>
              </tr>
              <tr>
                <td className="p-3 pr-4 align-top">
