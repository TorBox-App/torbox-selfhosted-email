...
vi.mock("@wraps/db", () => {
  ...
  apiKey: { id: "id", keyHash: "key_hash", organizationId: "organization_id", createdBy: "created_by", expiresAt: "expires_at", lastUsedAt: "last_used_at" },
  session: { id: "id", token: "token", userId: "user_id", expiresAt: "expires_at", activeOrganizationId: "active_organization_id" },
  member: { userId: "user_id", organizationId: "organization_id", role: "role" },
  subscription: { referenceId: "reference_id", plan: "plan", status: "status" },
    eq: vi.fn((a, b) => ({ eq: [a, b] })),
