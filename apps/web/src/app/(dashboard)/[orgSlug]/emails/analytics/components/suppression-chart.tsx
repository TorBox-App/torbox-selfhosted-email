...
export function SuppressionChart({ orgSlug }: { orgSlug: string }) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useSuppressionData(orgSlug, days);

  const chartData = suppressionData || [];

  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle>Suppression Rate</CardTitle>
        <CardDescription>
          <span className="@[540px]/card:block hidden">
            Emails blocked due to suppression list over time
          </span>
          <span className="@[540px]/card:hidden">Suppression rate (%)</span>
        </CardDescription>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        {isLoading ? (
          ...
          <div className="flex h-[300px] items-center justify-center text-muted-foreground text-sm">
            Failed to load suppression data
          </div>
        ) : chartData.length === 0 ? (
          <div className="flex h-[300px] items-center justify-center text-muted-foreground text-sm">
            No suppression data available for this time period
          </div>
        ) : (
          <ChartContainer
            ...
            <AreaChart data={chartData}>
              <defs>
                ...
                >
                  <stop
                    offset="5%"
                    stopColor="var(--color-suppressionRate)"
                    stopOpacity={0.4}
                  />
                  <stop
                    offset="95%"
              ...
              />
              <Area
                dataKey="suppressionRate"
                fill="url(#fillSuppressionRate)"
                stroke="var(--color-suppressionRate)"
                strokeWidth={2}
                type="monotone"
              />
            </AreaChart>
